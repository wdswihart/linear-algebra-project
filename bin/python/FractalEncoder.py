...
class FractalEncoder:
	...
	def __setupEncoder(self, filename):
		...
		downImage = image.Scale(image.GetWidth()/2, image.GetHeight()/2)

		#Partition the original and downscaled images into
		#squares of equal sizes
		rBoxes = self.__createBoxes(image.GetWidth(), image.GetHeight())
		dBoxes = self.__createBoxes(downImage.GetWidth(), downImage.GetHeight())
		
		self.__width = image.GetWidth()
		self.__height = image.GetHeight()
		...
		self._rBlocks = [RangeBlock(image.Resize(box, box, 1).GetData() for box in rBoxes]
	...
	def encodeImage(self, filename):
			...
			self.__writeToFIle(filename + '.fif')
		
# MAIN

blockSize = 4 # Must be a power of two; a lower blocksize means more details
encoder = FractalEncoder(blockSize)

# Benchmark the encoding.
start = time.time()
encoder.encodeImage('../../res/Lena.jpg')
